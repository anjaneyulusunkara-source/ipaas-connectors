class XurrentConnector < IPaaS::Connector::Definition
  ...
  OUTPUT_STATIC_IDS = [:ratelimit, :costlimit, :request_id].freeze

  # Keys a stored bundle part must carry to be usable; a part missing any (an
  # entry written by an older connector that shaped bundles differently) is
  # ignored so run rebuilds from the schema instead of reading an incompatible shape.
  BUNDLE_REQUIRED_KEYS = {
    [:query, 'in'] => %w[is_connection field_selection input_fields].freeze,
  ...
  connector '01930641-94f0-7d88-941f-cd0f542b75b9' do
    ...
    # ──────────────────────────────────────────────
    # Defined before the actions so the eagerly-built input_schema finds them registered. The
    # caching machinery lives in the shared ArtifactCache; these thin wrappers supply the
    # connection store and the connector-specific selection inputs.

    # Scope the cache to the outbound connection so all actions on it share one schema. Nil for an
    # unconfigured action — the accessors then no-op so the "configure the connection" notice shows.
    ...
    action '019ce240-76c9-75d1-beac-8c07b2325e76' do
      ...
      # schema_data (and the bundle) live only in this helper's frame, so the
      # input_schema block's after_update closure never captures them. Capturing
      # the parsed gql_schema would pin the multi-MB blob on the cached Schema per
      # solution version (causing OOM).
      ...
      # Static top-level input fields the builder always rebuilds; never serialized.
      QUERY_STATIC_INPUT_IDS = [:object, :page_size, :max_results, :refresh_schema].freeze

      # The 'in' bundle: the run shape plus the dynamic input descriptors collected
      # from the just-built LIVE input schema. Collecting from the live schema keeps
      # the warm input build structurally identical to the cold one with no rebuild.
      ...
      # The schema-derived run inputs (connection shape, type names, field selection,
      # arg type-refs) that run assembles the final query and variables from.
      helper :build_query_bundle_shape do |schema_data, object_name|
        ...
        if node_type_name.present?
          ...
          %w[view filter order].each do |arg_name|
            arg = helpers.find_gql_arg(gql_args, arg_name)
            bundle["#{arg_name}_type_ref"] = GqlQuery.gql_type_ref_string(arg['type']) if arg
          end
          bundle
        else
      ...
      # field_selection and the arg type-refs come from the bundle (built once from
      # the schema); the page_size/cursor literals and the present?/compact_hash
      # variable gates are assembled here from live runtime values.
      helper :run_connection_query do |bundle, object_name|
        ...
        if input[:view].present? && bundle['view_type_ref']
          var_defs << "$view: #{bundle['view_type_ref']}"
          query_params << 'view: $view'
          variables['view'] = input[:view]
        end

        filter_hash = CompactHash.compact_hash(input[:filter])
        if filter_hash.present? && bundle['filter_type_ref']
          var_defs << "$filter: #{bundle['filter_type_ref']}"
          query_params << 'filter: $filter'
          variables['filter'] = filter_hash
        end

        if input[:order].present? && bundle['order_type_ref']
          var_defs << "$order: #{bundle['order_type_ref']}"
      ...
      helper :run_simple_query do |bundle, object_name|
        query = "{ #{object_name} { #{bundle['field_selection']} } }"
    ...
    action '019ce240-76c9-7847-9dfa-a48d104515b3' do
      ...
      # helper is registered when first called. schema_data and the bundle stay local
      # to this helper and are never captured by the after_update closure (which lives
      # on the cached Schema) — see build_query_input_fields above.
      ...
      # Miss path for run (the schema-regeneration warm-up did not run, e.g. a cold
      # worker whose parse-time introspection failed): introspect, build the run shape,
      # and warm the 'in' bundle + root options from the live input schema so later
      # runs and actions read it without re-parsing.
    ...
    # Shared after_update logic for query and mutation actions: clears the derived caches
    ...
    # Returns the connection-scoped schema, fetching it once on a miss. A present
    # gql_schema entry is the single source of truth; an absent one re-introspects.
