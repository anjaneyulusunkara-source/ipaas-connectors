...
describe 'Microsoft Outlook Renew Mail Subscription Action', :action, :microsoft_outlook do
  let(:action_template_id) { '8fb16815-5d47-4b96-8acb-7b410138d41e' }
  ...
  it 'fails when no subscription has been stored for that runbook' do
  ...
  context 'when a subscription is stored' do
    before do
      ...
      outbound_connection.store.write("outlook_mail_subscription_id-#{target_runbook_uuid}", 'sub-1')
    end

    it 'renews the stored subscription and returns its new expiration' do
      stub = stub_request(:patch, 'https://graph.microsoft.com/v1.0/subscriptions/sub-1')
             .with(body: hash_including('expirationDateTime'))
    ...
    it 'treats a success response with an empty body as an empty result' do
      stub_request(:patch, 'https://graph.microsoft.com/v1.0/subscriptions/sub-1').to_return(status: 200, body: '')
